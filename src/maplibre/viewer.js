...
import { INITIAL_VIEW, styleIds, Z_RAW_POLYS } from './config.js';
import { addBasemap } from './baselayer.js';
import { addVectorSources, addPolygonLayers } from './layers.js';
...
function popupHTML(props) {
    ...
    const rows = fields.filter(k => k in (props||{}))
        .map(k => `<div><b>${k}</b>: ${props[k]}</div>`).join('');
